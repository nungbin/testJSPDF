...
var copyCSSStyles = exports.copyCSSStyles = function copyCSSStyles(style, target, bIsInlineFormElement) {
    ...
    for (var i = style.length; i >= 0; i--) {
        ...
        if (property !== 'content') {
            ...
            // With the latest chrome update, the text in the text area/ input are not coming up. This is due to Left and Top css properties being set as auto in this method for input elements.
            // Original lines:
            // target.style.setProperty(property, style.getPropertyValue(property));
            // Modified lines:
            if (bIsInlineFormElement) {
...
var inlineFormElement = function inlineFormElement(value, node, container, allowLinebreak) {
    ...
    if (value.length > 0 && body) {
        var wrapper = node.ownerDocument.createElement('html2canvaswrapper');
        // ##### BEGIN: MODIFIED BY SAP
        // With the latest chrome update, the text in the text area/ input are not coming up.
        // Original lines:
        //(0, _Util.copyCSSStyles)(node.ownerDocument.defaultView.getComputedStyle(node, null), wrapper);
        // Modified lines:
        (0, _Util.copyCSSStyles)(node.ownerDocument.defaultView.getComputedStyle(node, null), wrapper, true);
        // ##### END: MODIFIED BY SAP
        wrapper.style.position = 'absolute';
